...
export default class Sketch extends React.Component{
  ...
  constructor(options) {
    ...
    str.containers = options.dom;
    str.img = this.containers.querySelector(".landingImage");
    str.width = this.containers.offsetWidth;
    str.height = this.containers.offsetHeight;
    str.renderer = new THREE.WebGLRenderer();
    str.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
    str.renderer.setSize(this.width, this.height);
    str.renderer.setClearColor(0xeeeeee, 1);
    str.renderer.physicallyCorrectLights = true;
    str.renderer.outputEncoding = THREE.sRGBEncoding;

    str.containers.appendChild(this.renderer.domElement);

    str.camera = new THREE.PerspectiveCamera(
      70,
    ...
    str.camera = new THREE.OrthographicCamera(frustumSize / -2, frustumSize / 2, frustumSize / 2, frustumSize / -2, -1000, 1000);
    str.camera.position.set(0, 0, 2);
  ...
  getValue(value){
    return parseFloat(this.containers.getAttribute("data-"+value))
  }
